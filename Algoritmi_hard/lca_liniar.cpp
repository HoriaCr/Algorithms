...
int log2[2 * MAXN]; // precalced logarithms (floored values) // walk graph 
...
void build_lca() {
	int sz = (int) a.size();
	for (int i = 2, j = 0; i < sz; ++i) { // precalc logarithms
		log2[i] = log2[i>>1] + 1;
	}
	block = (log(sz) + 1) / 2;
	int blocks = sz / block + (sz % block ? 1 : 0); // precalc in each block and build sparse table
	memset(bt, 255, sizeof bt);
	for (int i = 0, bl = 0, j = 0; i < sz; ++i, ++j) {
		if (j == block) j = 0, ++bl;
		if (bt[bl][0] == -1 || min_h(i, bt[bl][0]) == i) bt[bl][0] = i;
	}
	for (int j = 1; j <= log(sz); ++j) for (int i = 0; i < blocks; ++i) {
		int ni = i + (1 << (j - 1));
